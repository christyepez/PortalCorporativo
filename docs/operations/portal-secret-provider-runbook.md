# Portal Secret Provider Runbook

## Purpose

Validate that Secret Provider preparation is documented and production providers remain disabled.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\check-portal-secret-provider-guardrails.ps1
powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\verify-portal-secret-provider-preparation.ps1
```
