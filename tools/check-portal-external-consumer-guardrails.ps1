...
try {
  ...
  if ($compose -match '(?im)^\s{2}(crm|financiero|financial)[-_a-z0-9]*\s*:') {
    Add-Failure 'Docker Compose defines CRM/Financiero services during external consumer onboarding preparation.'
  }
  ...
  Write-Host 'Portal External Consumer guardrails OK: no CRM/Financiero runtime coupling, productive routes, shared consumer databases, private URLs, real secrets or browser token storage detected.'
}
finally {
  Pop-Location
}
