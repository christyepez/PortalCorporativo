...
try {
  ...
  if ($compose -match '(?im)^\s{2}(crm|financiero|financial)[-_a-z0-9]*\s*:') {
    Add-Failure 'Docker Compose defines CRM/Financiero services during controlled NonProduction deployment.'
  }
  ...
  Write-Host 'Portal controlled NonProduction deployment guardrails OK: production, real providers, secrets, private URLs, browser token storage, consumer routes and runtime coupling are disabled.'
}
finally {
  Pop-Location
}
