...
if ($certificateFiles) {
  Add-Failure 'Certificate or key files found in controlled runtime paths.'
}
...
$unsafePatterns = $files |
  ...
  Where-Object { $_.Line -notmatch 'Select-String -Pattern|notmatch|match|unsafePatterns|LOCAL_ONLY_PLACEHOLDER|PLACEHOLDER_CHANGE_ME|No real|NoGo' }
if ($unsafePatterns) {
...
if (Test-Path $composePath) {
  ...
  if ($compose -match '(?im)^\s*(crm|financiero|financial)[-_a-z0-9]*\s*:') {
    Add-Failure 'Docker Compose defines CRM/Financiero services during Portal Sprint 9.'
  }
}
...
Write-Host 'Portal controlled runtime guardrails OK: no real .env, secrets, certificates, private URLs, consumer runtime coupling, or duplicate SQL Server detected.'
