...
public sealed class ContentDocument
{
    ...
    private ContentDocument(Guid id, string tenantId, string moduleCode, string fileName, string contentType, long length, string sha256, bool isActive, DateTimeOffset createdAt)
    {
        Id = id;
        TenantId = tenantId;
    ...
    public static ContentDocument Create(string tenantId, string moduleCode, string fileName, string contentType, long length, string sha256, DateTimeOffset now)
    {
        if (string.IsNullOrWhiteSpace(tenantId) || tenantId.Trim().Length > 64) throw new ArgumentException("TenantId is required and must be <= 64 characters.", nameof(tenantId));
