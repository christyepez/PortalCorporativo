...
public static class ContentEndpoints
{
    public static IEndpointRouteBuilder MapContentEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/documents", async (string? moduleCode, bool? isActive, ContentService service, IPortalTenantContext tenantContext, CancellationToken cancellationToken)
            => Results.Ok(await service.ListAsync(tenantContext.TenantId, moduleCode, isActive, cancellationToken)))
            .RequireAuthorization(PortalPermissions.ContentRead);

        group.MapGet("/documents/{id:guid}", async (Guid id, ContentService service, IPortalTenantContext tenantContext, CancellationToken cancellationToken)
            => await service.GetAsync(tenantContext.TenantId, id, cancellationToken) is { } document ? Results.Ok(document) : Results.NotFound())
            .RequireAuthorization(PortalPermissions.ContentRead);

        group.MapGet("/documents/{id:guid}/download", async (Guid id, ContentService service, IPortalTenantContext tenantContext, CancellationToken cancellationToken) =>
        {
            var stored = await service.DownloadAsync(tenantContext.TenantId, id, cancellationToken);
