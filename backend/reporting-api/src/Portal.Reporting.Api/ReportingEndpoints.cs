...
public static class ReportingEndpoints
{
    public static IEndpointRouteBuilder MapReportingEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/reports/{key}/execute", async (string key, ExecuteReportRequest request, ReportingService service, CancellationToken cancellationToken) =>
        {
            try
            {
                return await service.ExecuteAsync(key, request, cancellationToken) is { } result ? Results.Ok(result) : Results.NotFound();
            }
