...
{
    Task<MenuDefinition?> GetMenuAsync(string tenant, Guid id, CancellationToken ct); Task<MenuDefinition?> GetMenuByModuleAsync(string tenant, string module, CancellationToken ct);
    Task<MenuItem?> GetItemAsync(string tenant, Guid id, CancellationToken ct); Task<IReadOnlyCollection<MenuItem>> GetItemsAsync(string tenant, Guid menuId, CancellationToken ct);
...
public sealed class MenuService(IMenuStore store, IPermissionChecker permissions, IMenuChangeRecorder recorder, IPortalTenantContext tenantContext)
{
    ...
    { if (await store.GetMenuAsync(tenantContext.TenantId, r.MenuId, ct) is null) return NotFound(); try { var item = MenuItem.Create(r.MenuId, r.ParentId, r.Code, r.Label, r.Route, r.Icon, r.Order, r.ResourceKey, r.PermissionCode, r.MetadataJson); await store.AddAsync(item, ct); await store.SaveChangesAsync(ct); await Changed(item, "Created", correlation, ct); return Result<MenuItemResponse>.Success(Map(item)); } catch (Exception e) when (e is ArgumentException or System.Text.Json.JsonException) { return Result<MenuItemResponse>.Failure("menu.validation", e.Message); } }
    ...
    { var item = await store.GetItemAsync(tenantContext.TenantId, id, ct); if (item is null) return NotFound(); item.Update(item.Label, item.Route, item.Icon, r.ResourceKey, r.PermissionCode, item.MetadataJson); await store.SaveChangesAsync(ct); await Changed(item, "PermissionAssigned", correlation, ct); return Result<MenuItemResponse>.Success(Map(item)); }
    public async Task<Result<MenuItemResponse>> SetActiveAsync(Guid id, bool active, string correlation, CancellationToken ct)
    { var item = await store.GetItemAsync(tenantContext.TenantId, id, ct); if (item is null) return NotFound(); item.SetActive(active); await store.SaveChangesAsync(ct); await Changed(item, active ? "Activated" : "Deactivated", correlation, ct); return Result<MenuItemResponse>.Success(Map(item)); }
    public async Task<Result<bool>> ReorderAsync(ReorderMenuRequest request, string correlation, CancellationToken ct)
    { foreach (var change in request.Items) { var item = await store.GetItemAsync(tenantContext.TenantId, change.ItemId, ct); if (item is null) return Result<bool>.Failure("menu.not_found", "Menu item was not found."); item.Reorder(change.Order, change.ParentId); } await store.SaveChangesAsync(ct); await recorder.RecordAsync("reordered", "menu", new { request.Items, correlation }, correlation, ct); return Result<bool>.Success(true); }
    ...
    { var menu = await store.GetMenuByModuleAsync(tenantContext.TenantId, module.ToLowerInvariant(), ct); if (menu is null) return Result<IReadOnlyCollection<MenuItemResponse>>.Failure("menu.not_found", "Menu was not found."); var visible = new List<MenuItemResponse>(); foreach (var item in (await store.GetItemsAsync(tenantContext.TenantId, menu.Id, ct)).Where(x => x.IsActive).OrderBy(x => x.Order)) { var action = item.PermissionCode.Split('.').Last(); if (await permissions.HasPermissionAsync(user, item.ResourceKey, action, ct)) visible.Add(Map(item)); } return Result<IReadOnlyCollection<MenuItemResponse>>.Success(visible); }
