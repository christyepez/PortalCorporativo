...
public sealed class SecurityRevocationService(ISecurityRevocationStore store)
{
    public async Task<Result<bool>> RevokeRoleFromUserAsync(Guid userId, Guid roleId, CancellationToken cancellationToken)
    {
        ...
        if (await store.FindRoleAsync(TenantIds.Default, roleId, cancellationToken) is null)
            return Result<bool>.Failure("security.role.not_found", "Role was not found.");

        var removed = await store.RemoveUserRoleAsync(TenantIds.Default, userId, roleId, cancellationToken);
        return removed
    ...
    public async Task<Result<bool>> RevokePermissionFromRoleAsync(Guid roleId, Guid permissionId, CancellationToken cancellationToken)
    {
        if (await store.FindRoleAsync(TenantIds.Default, roleId, cancellationToken) is null)
            return Result<bool>.Failure("security.role.not_found", "Role was not found.");
        if (await store.FindPermissionAsync(TenantIds.Default, permissionId, cancellationToken) is null)
            return Result<bool>.Failure("security.permission.not_found", "Permission was not found.");

        var removed = await store.RemoveRolePermissionAsync(TenantIds.Default, roleId, permissionId, cancellationToken);
        return removed
            ? Result<bool>.Success(true)
