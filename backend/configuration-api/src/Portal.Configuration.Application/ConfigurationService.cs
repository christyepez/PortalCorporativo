...
{
    Task<ConfigurationItem?> GetAsync(string tenant, Guid id, CancellationToken ct);
...
public sealed class ConfigurationService(IConfigurationStore store, IPlatformChangeRecorder recorder, IClock clock, IPortalTenantContext tenantContext)
{
    public async Task<Result<ConfigurationResponse>> CreateAsync(CreateConfigurationRequest request, string correlationId, CancellationToken ct)
    {
        try
        {
            var item = ConfigurationItem.Create(request.Key, (ConfigurationScope)request.Scope, tenantContext.TenantId, request.ModuleCode,
    ...
    {
        var item = await store.GetAsync(tenantContext.TenantId, id, ct); if (item is null) return NotFound();
    ...
    {
        var candidates = await store.FindCandidatesAsync(tenantContext.TenantId, key.ToLowerInvariant(), module?.ToLowerInvariant(), user, ct);
    ...
    { var items = await store.GetByScopeAsync((ConfigurationScope)scope, tenantContext.TenantId, module, user, ct); return Result<IReadOnlyCollection<ConfigurationResponse>>.Success(items.Select(Map).ToArray()); }
